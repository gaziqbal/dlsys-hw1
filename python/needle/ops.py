...
class EWiseDiv(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        lhs, rhs = node.inputs
        return divide(out_grad, rhs), multiply(out_grad, divide(negate(lhs), power_scalar(rhs, 2)))
        ### END YOUR SOLUTION
...
class Transpose(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if not axes:
            input = node.inputs[0]
            input_num_axes = len(input.shape)
            axes = (input_num_axes-1, input_num_axes-2)
        axes = axes[1], axes[0]
...
class Summation(TensorOp):
    def __init__(self, axes: Optional[tuple] = None):
        assert axes is None or isinstance(axes, tuple)
        self.axes = axes
...
class Log(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        input = node.inputs[0]
        return power_scalar(input, -1) * out_grad
        ### END YOUR SOLUTION
